// src/app/g/[slug]/welcome/page.tsx
...
export default async function WelcomePage({ params }: PageProps<{ slug: string }>) {
  ...
  const s = property.sections.find(x => x.kind === 'WELCOME') ?? null;
    ...
    <main className="p-6 max-w-xl mx-auto space-y-4">
      {property.logoDataUrl && (
        <div className="flex justify-center">
          <Image src={property.logoDataUrl} alt={`${property.name} logo`} width={120} height={120} unoptimized priority />
        </div>
      )}
      <h1 className="text-3xl font-bold" style={{ color: primary }}>{s?.title ?? 'Welcome'}</h1>
      ...
      <nav className="pt-2 text-sm">
        ...
        <a className="underline text-blue-600" href={`/g/${slug}/guide`}>Local Guide</a>
      </nav>
